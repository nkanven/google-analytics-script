...
Sample Usage:
  ...
  $ python analytics_api_v3.py

Also you can also get help on all the command-line flags the program
understands by running:

  $ python analytics_api_v3.py --help
"""
from __future__ import print_function

__author__ = 'nkanven@gmail.com (Nkondog Anselme venceslas)'
...
def print_results(results):
  ...
  if results.get('rows', []):
    for row in results.get('rows'):
      ...
      #print(time.ctime(os.path.getctime('analytics_pick')))
      #input()
      if 350 > int(users):
      ...
      try:
          with open('analytics_pick', 'rb') as fichier:
              mon_depickler = pickle.Unpickler(fichier)
              pik = mon_depickler.load()
      except Exception as e:
          with open('analytics_pick', 'wb') as f:
              my_pickler = pickle.Pickler(f)
              my_pickler.dump(int(users))
      print('pik = '+str(pik))

      if int(users) > int(pik):
          with open('analytics_pick', 'wb') as f:
              my_pickler = pickle.Pickler(f)
              my_pickler.dump(int(users))
...
if __name__ == '__main__':
    while True:
        ...
        try:
            ...
            print('Something weird happened but I skipped it')
            continue